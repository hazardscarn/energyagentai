...
def analyze_energy_efficiency(customer_id: str, analysis_days: int = 365) -> str:
    ...
    try:
        ...
        -- Enhanced daily patterns with time-based analysis
        daily_patterns AS (
          SELECT 
            sm.customer_id,
            sm.date,
            ...
            -- Night vs Day usage (8PM-8AM vs 8AM-8PM)
            ...
            -- Peak hours analysis (high usage periods)
        ...
        time_based_analysis AS (
          SELECT 
            customer_id,
            -- Early period (first 30% of data)
            AVG(CASE WHEN ROW_NUMBER() OVER (ORDER BY date) <= COUNT(*) OVER () * 0.3 
                     THEN daily_usage END) as early_avg_usage,
            AVG(CASE WHEN ROW_NUMBER() OVER (ORDER BY date) <= COUNT(*) OVER () * 0.3 
                     THEN min_interval_usage END) as early_baseline,
            
            -- Late period (last 30% of data)  
            AVG(CASE WHEN ROW_NUMBER() OVER (ORDER BY date) > COUNT(*) OVER () * 0.7 
                     THEN daily_usage END) as late_avg_usage,
            AVG(CASE WHEN ROW_NUMBER() OVER (ORDER BY date) > COUNT(*) OVER () * 0.7 
                     THEN min_interval_usage END) as late_baseline,
        ...
        SELECT 
          *,
          ...
          CASE 
            WHEN baseline_ratio > 0.6 THEN 15  -- Very high baseline (phantom loads)
            WHEN baseline_ratio > 0.4 THEN 30  -- High baseline
            WHEN baseline_ratio > 0.25 THEN 60  -- Moderate baseline
            ELSE 85  -- Good baseline
          END as baseline_score,
          
          CASE 
            WHEN variability_coefficient > 0.8 THEN 10  -- Very inconsistent
            WHEN variability_coefficient > 0.5 THEN 30  -- Inconsistent
            WHEN variability_coefficient > 0.3 THEN 60  -- Somewhat consistent
            ELSE 90  -- Very consistent
          END as consistency_score,
          
          CASE 
            WHEN peak_ratio > 8 THEN 5   -- Extreme peaks
            WHEN peak_ratio > 5 THEN 25  -- High peaks
            WHEN peak_ratio > 3 THEN 55  -- Moderate peaks
            ELSE 85  -- Reasonable peaks
          END as peak_efficiency_score,
          
          CASE 
            WHEN usage_increased AND baseline_degraded THEN 10  -- Major degradation
            WHEN usage_increased OR baseline_degraded THEN 40   -- Some degradation
            WHEN ABS(usage_change_percent) < 5 THEN 90          -- Stable
            ELSE 70  -- Minor changes
          END as stability_score,
          
          CASE 
            WHEN avg_daily_faults > 0.1 THEN 20  -- Frequent equipment issues
            WHEN avg_daily_faults > 0.05 THEN 60  -- Some equipment issues
            ELSE 95  -- Minimal equipment issues
